...
export enum AccountBalancesApiApiKeys {
}

export class AccountBalancesApi {
    ...
    public setApiKey(key: AccountBalancesApiApiKeys, value: string) {
        (this.authentications as any)[AccountBalancesApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async getAccountBalance (companySlug: string, accountCode: string, date: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: AccountBalance;  }> {
        const localVarPath = this.basePath + '/companies/{companySlug}/accountBalances/{accountCode}'
        ...
        if (companySlug === null || companySlug === undefined) {
            throw new Error('Required parameter companySlug was null or undefined when calling getAccountBalance.');
        }

        // verify required parameter 'accountCode' is not null or undefined
        if (accountCode === null || accountCode === undefined) {
            throw new Error('Required parameter accountCode was null or undefined when calling getAccountBalance.');
        }

        // verify required parameter 'date' is not null or undefined
        if (date === null || date === undefined) {
            throw new Error('Required parameter date was null or undefined when calling getAccountBalance.');
        }
     ...
     */
    public async getAccountBalances (companySlug: string, date: string, fromAccount?: number, toAccount?: number, page?: number, pageSize?: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Array<AccountBalance>;  }> {
        const localVarPath = this.basePath + '/companies/{companySlug}/accountBalances'
            .replace('{' + 'companySlug' + '}', encodeURIComponent(String(companySlug)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['application/json'];
        ...
        // verify required parameter 'companySlug' is not null or undefined
        if (companySlug === null || companySlug === undefined) {
            throw new Error('Required parameter companySlug was null or undefined when calling getAccountBalances.');
        }

        // verify required parameter 'date' is not null or undefined
        if (date === null || date === undefined) {
            throw new Error('Required parameter date was null or undefined when calling getAccountBalances.');
        }

        if (fromAccount !== undefined) {
            localVarQueryParameters['fromAccount'] = ObjectSerializer.serialize(fromAccount, "number");
        }

        if (toAccount !== undefined) {
            localVarQueryParameters['toAccount'] = ObjectSerializer.serialize(toAccount, "number");
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: Array<AccountBalance>;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "Array<AccountBalance>");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
